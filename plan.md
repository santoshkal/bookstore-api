...
- No tests exist → every code PR is verified with `go build ./... && go vet ./...` (from `pkg/`) **plus a manual smoke run** against a local Postgres or the Kind cluster. A change that builds but crashes at runtime is the failure mode to guard against.
